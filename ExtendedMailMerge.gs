...
function createSpreadsheetEditTrigger() {
  ...
  ScriptApp.newTrigger('handleEdit')
    ...
    .onEdit()
    .create();
  SpreadsheetApp.getUi().alert('Success!', 'The automated email trigger has been set up. Emails will now be sent automatically when you change a status.', SpreadsheetApp.getUi().ButtonSet.OK);
}
...
function handleEdit(e) {
  ...
  // **FIX 1a:** Trim whitespace from all headers when they are read.
  ...
  if (range.getColumn() === statusColIdx + 1) {
    ...
    const rowDataArray = sheet.getRange(range.getRow(), 1, 1, sheet.getLastColumn()).getValues()[0];
    
    // **FIX 1b:** Trim whitespace from all cell data as it's read.
    ...
    // Cache the data from this edit for debugging purposes.
    CacheService.getScriptCache().put('last_edited_row', JSON.stringify(rowObject), 3600);
...
function processEmailTrigger(rowObject, status, sheet, rowNum, headers) {
  ...
  try {
    ...
    const messageObject = fillInTemplateFromObject_(emailTemplate.message, rowObject, globalParameters, CONFIG.PLACEHOLDER_ALIASES);

    // Use a cleaned version of the recipient for the actual sending to be safe.
    const cleanRecipient = String(recipient).replace(/\s/g, '');

    GmailApp.sendEmail(cleanRecipient, messageObject.subject, messageObject.text, {
    ...
    SpreadsheetApp.getUi().alert(`An error occurred while sending an email: ${e.message}`);
  }
}
...
function clearCache() {
  ...
  SpreadsheetApp.getUi().alert('Cache Cleared', 'The cache for the "General" sheet has been cleared. The script will now read the latest data on the next edit.', SpreadsheetApp.getUi().ButtonSet.OK);
}
...
function debugLastEdit() {
  ...
  if (!lastRowJSON) {
    ui.alert('No Edit Data Found', 'Please edit a status in a row in the "Mailer" sheet first, then run this command again.', ui.ButtonSet.OK);
    return;
  }
  ...
  const htmlOutput = HtmlService.createHtmlOutput(`<p>Data for the last edited row. Check that "Fees" exists in the ROW DATA below with the correct value.</p><textarea rows="20" cols="80" readonly>${debugText}</textarea>`)
    .setWidth(600)
    .setHeight(400);
...
function getGlobalParameters_() {
  ...
  const data = generalSheet.getRange(2, 2, generalSheet.getLastRow() - 1, 3).getValues();
  const globalParams = {};
...
function getGmailTemplateFromDrafts_(subject_line) {
  try {
    ...
    if (!draft) throw new Error(`Draft with subject "${subject_line}" not found.`);
    ...
    return {
      message: { subject: msg.getSubject(), text: msg.getPlainBody(), html: htmlBody },
      attachments: attachments,
      inlineImages: inlineImagesObj
    };
  } catch (e) {
    throw new Error(`Oops - can't find or process Gmail draft with subject "${subject_line}". Error: ${e.message}`);
  }
...
/**
 * Fills a template string with data, checking global params, aliases, and direct column names.
 * @param {object} template The message object {subject, text, html}.
 * @param {object} rowData The data object for the current row from the "Mailer" sheet.
 * @param {object} globalData The global parameters from the "General" sheet.
 * @param {object} aliases The mapping of short names to full column names for the Mailer sheet.
...
function fillInTemplateFromObject_(template, rowData, globalData, aliases) {
  ...
  template_string = template_string.replace(/{{[^{}]+}}/g, key => {
    ...
    if (replacementData.hasOwnProperty(cleanedKey)) {
      let value = replacementData[cleanedKey] || "";
      
      // **FIX 2:** If the placeholder is for an email, strip all whitespace.
      if (cleanedKey === 'email' || cleanedKey === 'orgEmail') {
...
/**
 * Escape cell data to make it JSON safe.
 */
function escapeData_(str) {
    const stringifiedStr = typeof str === 'string' ? str : String(str);
...
function checkGmailSentStatus() {
  const ui = SpreadsheetApp.getUi();
  const columnPrompt = ui.prompt('Step 1: Select Column', 'Enter the exact name of the column containing the email addresses you want to check:', ui.ButtonSet.OK_CANCEL);
  ...
  const subjectPrompt = ui.prompt('Step 2: Specify Subject', 'Enter the exact subject line of the sent email you want to search for (e.g., "Payment Confirmation - Wieda Summer Hike 2025"):', ui.ButtonSet.OK_CANCEL);
  ...
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.MAILER_SHEET);
  if (!sheet) {
    ui.alert('Error', `Sheet with name "${CONFIG.MAILER_SHEET}" was not found. Please check the CONFIG.`, ui.ButtonSet.OK);
    return;
  }
  ...
  try {
    ...
    if (emailColIdx === -1) {
      ui.alert('Error', `Column "${columnToCheck}" was not found.`, ui.ButtonSet.OK);
      return;
    }

    const emailData = sheet.getRange(2, emailColIdx + 1, sheet.getLastRow() - 1, 1).getValues();
    const emailsToCheck = [...new Set(emailData.flat().filter(email => typeof email === 'string' && email.includes('@')))];
    if (emailsToCheck.length === 0) {
      ui.alert('No Emails Found', 'Could not find any valid email addresses in the specified column.', ui.ButtonSet.OK);
      return;
    }

    ui.alert('Starting Check', `Checking ${emailsToCheck.length} unique email addresses for the subject "${subjectToCheck}" from the last two months. This may take some time...`, ui.ButtonSet.OK);
    ...
    if (notSentList.length === 0) {
      ui.alert('All Clear!', `It appears an email with the subject "${subjectToCheck}" has been sent to every address on the list within the last two months.`, ui.ButtonSet.OK);
    } else {
      const htmlOutput = HtmlService.createHtmlOutput(`<p>Found ${notSentList.length} emails that were not sent an email with the subject "${subjectToCheck}" in the last two months:</p><textarea rows="15" cols="60" readonly>${notSentList.join('\n')}</textarea>`)
        .setWidth(500)
        .setHeight(350);
